"""
Zep图谱记忆更新服务（已迁移至 Graphiti + Neo4j）
...
class ZepGraphMemoryUpdater:
    ...
    监控模拟的actions日志，将新的agent活动实时更新到图谱中。
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        Args:
            ...
            api_key: 保留参数，已不再使用（迁移后无需 API Key）
        """
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        """批量发送活动到图谱（使用 Graphiti add_episode）"""
        if not activities:
            return
            ...
            try:
                run_async(
                    graphiti.add_episode(
                        name=f"sim_activity_{uuid4().hex[:8]}",
                        episode_body=combined_text,
                        source_description=f"Simulation activity - {platform}",
