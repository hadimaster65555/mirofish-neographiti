...
class OasisProfileGenerator:
    ...
    def __init__(
        ...
        # Graphiti 客户端用于检索丰富上下文（zep_api_key 参数保留以兼容调用方，不再使用）
        self.graph_id = graph_id
        self._graphiti = None  # 懒加载
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"图谱检索完成: {entity_name}, 获取 {len(results['facts'])} 条事实")
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user persona generation expert. Generate detailed, realistic personas for public opinion simulation that faithfully reproduce existing real-world situations. Must return valid JSON format; all string values must not contain unescaped newline characters."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, faithfully reproducing the existing real-world situation.
...
2. persona: Detailed persona description (2000-word plain text), must include:
   ...
   - Stance and viewpoints (attitude toward topics, content that might provoke or move them)
...
6. country: Country (e.g., "China", "USA")
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for an institution/group entity, faithfully reproducing the existing real-world situation.
...
2. persona: Detailed account profile description (2000-word plain text), must include:
   - Institution basic information (official name, nature of the institution, founding background, main functions)
   ...
   - Stance and attitude (official position on core topics, how disputes are handled)
   - Special notes (group profile represented, operational habits)
   - Institutional memory (important part of the persona: introduce this institution's connection to events and its existing actions and reactions in the events)
...
5. mbti: MBTI type describing account style, e.g., ISTJ represents rigorous and conservative
6. country: Country (e.g., "China", "USA")
